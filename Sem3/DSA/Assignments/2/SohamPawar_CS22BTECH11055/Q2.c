...
node* Make_tree(int pre[], int in[], int lower, int upper) {
	...
	node* root;
	root = Create_node(pre[root_index]);
	...
	root->left = Make_tree(pre, in, lower, index - 1);
	root->right = Make_tree(pre, in, index + 1, upper);
...
int main() {
	...
	root = Make_tree(preorder, inorder, 0, size - 1);
