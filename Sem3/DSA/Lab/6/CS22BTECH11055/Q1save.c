...
int* Generate_hash() {
	...
	coeff[0] = rand() % (2 * dataset_size) + 1;
	coeff[1] = rand() % (2 * dataset_size) + 1;
	return coeff;
}
...
int main () {
	...
	int input[dataset_size * 2];

	for(int i = 0 ; i < dataset_size ; i++) fscanf(file, "%d", &input[i]);
	for (int i = dataset_size ; i < dataset_size * 2 ; i++) input[i] = rand() % 1000 + 1;
	...
	while (square_sum >= c * dataset_size * 2) {
	...
	for (int i = 0 ; i < 2 * dataset_size ; i++) {
